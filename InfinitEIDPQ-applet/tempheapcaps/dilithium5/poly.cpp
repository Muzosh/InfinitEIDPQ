...
void PQCLEAN_DILITHIUM5_CLEAN_poly_uniform(poly* a, const uint8_t* seed, uint16_t nonce)
{
    ...
    uint8_t* buf = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(POLY_UNIFORM_NBLOCKS * STREAM128_BLOCKBYTES + 2)), MALLOC_CAP_DEFAULT);
...
void PQCLEAN_DILITHIUM5_CLEAN_poly_uniform_eta(poly* a, const uint8_t* seed, uint16_t nonce)
{
    ...
    uint8_t* buf = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(POLY_UNIFORM_ETA_NBLOCKS * STREAM256_BLOCKBYTES)), MALLOC_CAP_DEFAULT);
...
{
    uint8_t* buf = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(POLY_UNIFORM_GAMMA1_NBLOCKS * STREAM256_BLOCKBYTES)), MALLOC_CAP_DEFAULT);
    stream256_state state;
...
void PQCLEAN_DILITHIUM5_CLEAN_poly_challenge(poly* c, const uint8_t* seed)
{
    ...
    uint64_t signs;
    uint8_t* buf = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(SHAKE256_RATE)), MALLOC_CAP_DEFAULT);
    shake256incctx state;

    shake256_inc_init(&state);
    shake256_inc_absorb(&state, seed, SEEDBYTES);
    shake256_inc_finalize(&state);
    shake256_inc_squeeze(buf, sizeof(uint8_t) * SHAKE256_RATE, &state);
...
{
    unsigned int i;
    uint8_t* t = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(8)), MALLOC_CAP_DEFAULT);
    DBENCH_START();
...
{
    unsigned int i;
    uint32_t* t = (uint32_t*)heap_caps_malloc((sizeof(uint32_t)*(8)), MALLOC_CAP_DEFAULT);
    DBENCH_START();
...
{
    unsigned int i;
    uint32_t* t = (uint32_t*)heap_caps_malloc((sizeof(uint32_t)*(4)), MALLOC_CAP_DEFAULT);
    DBENCH_START();
