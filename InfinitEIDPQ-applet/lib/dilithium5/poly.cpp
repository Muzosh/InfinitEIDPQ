...
void PQCLEAN_DILITHIUM5_CLEAN_poly_ntt(poly* a)
{
    ...
    PQCLEAN_DILITHIUM5_CLEAN_ntt(a->coeffs);
...
void PQCLEAN_DILITHIUM5_CLEAN_poly_invntt_tomont(poly* a)
{
    ...
    PQCLEAN_DILITHIUM5_CLEAN_invntt_tomont(a->coeffs);
...
void PQCLEAN_DILITHIUM5_CLEAN_poly_pointwise_montgomery(poly* c, const poly* a, const poly* b)
{
    ...
    for (i = 0; i < N; ++i) {
        c->coeffs[i] =
            PQCLEAN_DILITHIUM5_CLEAN_montgomery_reduce((int64_t)a->coeffs[i] * b->coeffs[i]);
    }
 ...
 **************************************************/
static unsigned int rej_uniform(int32_t* a, unsigned int len, const uint8_t* buf,
                                unsigned int buflen)
{
 ...
 *              - uint8_t* seed: byte array with seed of length SEEDBYTES
 *              - uint16_t nonce: 2-byte nonce
 **************************************************/
#define POLY_UNIFORM_NBLOCKS ((768 + STREAM128_BLOCKBYTES - 1) / STREAM128_BLOCKBYTES)
void PQCLEAN_DILITHIUM5_CLEAN_poly_uniform(poly* a, const uint8_t* seed, uint16_t nonce)
{
    unsigned int i, ctr, off;
    unsigned int buflen = POLY_UNIFORM_NBLOCKS * STREAM128_BLOCKBYTES;
    std::unique_ptr<uint8_t[]> buf(new uint8_t[POLY_UNIFORM_NBLOCKS * STREAM128_BLOCKBYTES + 2]);
    stream128_state state;

    stream128_init(&state, seed, nonce);
    stream128_squeezeblocks(buf.get(), POLY_UNIFORM_NBLOCKS, &state);

    ctr = rej_uniform(a->coeffs, N, buf.get(), buflen);

    while (ctr < N) {
        off = buflen % 3;
        for (i = 0; i < off; ++i) {
            buf.get()[i] = buf[buflen - off + i];
        }

        stream128_squeezeblocks(buf.get() + off, 1, &state);
        buflen = STREAM128_BLOCKBYTES + off;
        ctr += rej_uniform(a->coeffs + ctr, N - ctr, buf.get(), buflen);
    }
 ...
 *
 * Arguments:   - poly *a: pointer to output polynomial
 *              - uint8_t* seed(new uint8_t[]): byte array with seed of length CRHBYTES
 *              - uint16_t nonce: 2-byte nonce
 **************************************************/
#define POLY_UNIFORM_ETA_NBLOCKS ((136 + STREAM256_BLOCKBYTES - 1) / STREAM256_BLOCKBYTES)
void PQCLEAN_DILITHIUM5_CLEAN_poly_uniform_eta(poly* a, const uint8_t* seed, uint16_t nonce)
{
    unsigned int ctr;
    unsigned int buflen = POLY_UNIFORM_ETA_NBLOCKS * STREAM256_BLOCKBYTES;
    std::unique_ptr<uint8_t[]> buf(new uint8_t[POLY_UNIFORM_ETA_NBLOCKS * STREAM256_BLOCKBYTES]);
    stream256_state state;

    stream256_init(&state, seed, nonce);
    stream256_squeezeblocks(buf.get(), POLY_UNIFORM_ETA_NBLOCKS, &state);

    ctr = rej_eta(a->coeffs, N, buf.get(), buflen);

    while (ctr < N) {
        stream256_squeezeblocks(buf.get(), 1, &state);
        ctr += rej_eta(a->coeffs + ctr, N - ctr, buf.get(), STREAM256_BLOCKBYTES);
    }
 ...
 *
 * Arguments:   - poly *a: pointer to output polynomial
 *              - uint8_t* seed: byte array with seed of length CRHBYTES
 *              - uint16_t nonce: 16-bit nonce
 **************************************************/
#define POLY_UNIFORM_GAMMA1_NBLOCKS                                                                \
    ((POLYZ_PACKEDBYTES + STREAM256_BLOCKBYTES - 1) / STREAM256_BLOCKBYTES)
void PQCLEAN_DILITHIUM5_CLEAN_poly_uniform_gamma1(poly* a, const uint8_t* seed, uint16_t nonce)
{
    std::unique_ptr<uint8_t[]> buf(new uint8_t[POLY_UNIFORM_GAMMA1_NBLOCKS * STREAM256_BLOCKBYTES]);
    stream256_state state;

    stream256_init(&state, seed, nonce);
    stream256_squeezeblocks(buf.get(), POLY_UNIFORM_GAMMA1_NBLOCKS, &state);
    stream256_release(&state);
    PQCLEAN_DILITHIUM5_CLEAN_polyz_unpack(a, buf.get());
}
 ...
 *
 * Arguments:   - poly *c: pointer to output polynomial
 *              - uint8_t* mu(new uint8_t[]): byte array containing seed of length SEEDBYTES
 **************************************************/
void PQCLEAN_DILITHIUM5_CLEAN_poly_challenge(poly* c, const uint8_t* seed)
{
    unsigned int i, b, pos;
    uint64_t signs;
    std::unique_ptr<uint8_t[]> buf(new uint8_t[SHAKE256_RATE]);
    shake256incctx state;

    shake256_inc_init(&state);
    shake256_inc_absorb(&state, seed, SEEDBYTES);
    shake256_inc_finalize(&state);
    shake256_inc_squeeze(buf.get(), sizeof(uint8_t) * SHAKE256_RATE, &state);

    signs = 0;
    for (i = 0; i < 8; ++i) {
        signs |= (uint64_t)buf.get()[i] << 8 * i;
    }
    pos = 8;
...
{
    unsigned int i;
    std::unique_ptr<uint8_t[]> t(new uint8_t[8]);
    DBENCH_START();
...
{
    unsigned int i;
    std::unique_ptr<uint32_t[]> t(new uint32_t[8]);
    DBENCH_START();
...
{
    unsigned int i;
    std::unique_ptr<uint32_t[]> t(new uint32_t[4]);
    DBENCH_START();
