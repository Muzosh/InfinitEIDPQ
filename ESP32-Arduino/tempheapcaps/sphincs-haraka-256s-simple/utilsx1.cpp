 ...
 */
void treehashx1(uint8_t* root, uint8_t* auth_path, const spx_ctx* ctx, uint32_t leaf_idx,
                uint32_t idx_offset, uint32_t tree_height,
                void (*gen_leaf)(uint8_t* /* Where to write the leaves */, const spx_ctx* /* ctx */,
                ...
                uint32_t tree_addr[8], void* info)
{
    ...
    *max_idx = (uint32_t)((1 << tree_height) - 1);
    ...
    for (*idx = 0;; (*idx)++) {
        uint8_t* current = (uint8_t*)heap_caps_malloc(
            (sizeof(uint8_t) * (2 * SPX_N)), MALLOC_CAP_DEFAULT); /* Current logical node is at */
        ...
        uint32_t* internal_idx_offset =
            (uint32_t*)heap_caps_malloc(sizeof(uint32_t), MALLOC_CAP_DEFAULT);
        ...
        *internal_leaf = leaf_idx;
        uint32_t* h = (uint32_t*)heap_caps_malloc(
            sizeof(uint32_t), MALLOC_CAP_DEFAULT); /* The height we are in the Merkle tree */
