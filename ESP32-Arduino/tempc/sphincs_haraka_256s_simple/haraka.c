...
static void haraka_S_absorb(uint8_t *s, unsigned int r,
...
static void haraka_S_squeezeblocks(uint8_t *h, unsigned long long nblocks,
                                   uint8_t *s, unsigned int r,
                                   const spx_ctx *ctx) {
    while (nblocks > 0) {
        haraka512_perm(s, s, ctx);
...
void haraka_S(uint8_t *out, unsigned long long outlen,
...
void haraka512_perm(uint8_t *out, const uint8_t *in,
                    const spx_ctx *ctx) {
    uint32_t w[16];
    uint64_t q[8], tmp_q;
    unsigned int i, j;
