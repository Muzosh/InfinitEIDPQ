...
{
    size_t i;
    std::unique_ptr<uint8_t[]> t(new uint8_t[200]);
...
void shake128(uint8_t* output, size_t outlen, const uint8_t* input, size_t inlen)
{
    ...
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHAKE128_RATE]);
    shake128ctx s;
    ...
    if (outlen) {
        shake128_squeezeblocks(t.get(), 1, &s);
...
void shake256(uint8_t* output, size_t outlen, const uint8_t* input, size_t inlen)
{
    ...
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHAKE256_RATE]);
    shake256ctx s;
    ...
    if (outlen) {
        shake256_squeezeblocks(t.get(), 1, &s);
        for (size_t i = 0; i < outlen; ++i) {
            output[i] = t.get()[i];
        }
    }
...
{
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHA3_256_RATE]);
    keccak_inc_finalize(state->ctx, SHA3_256_RATE, 0x06);

    keccak_squeezeblocks(t.get(), 1, state->ctx, SHA3_256_RATE);
...
{
    std::unique_ptr<uint64_t[]> s(new uint64_t[25]);
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHA3_256_RATE]);

    /* Absorb input */
    keccak_absorb(s.get(), SHA3_256_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t.get(), 1, s.get(), SHA3_256_RATE);
...
{
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHA3_384_RATE]);
    keccak_inc_finalize(state->ctx, SHA3_384_RATE, 0x06);

    keccak_squeezeblocks(t.get(), 1, state->ctx, SHA3_384_RATE);
...
void sha3_384(uint8_t* output, const uint8_t* input, size_t inlen)
{
    ...
    /* Absorb input */
    keccak_absorb(s.get(), SHA3_384_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t.get(), 1, s.get(), SHA3_384_RATE);
...
{
    std::unique_ptr<uint8_t[]> t(new uint8_t[SHA3_512_RATE]);
    keccak_inc_finalize(state->ctx, SHA3_512_RATE, 0x06);

    keccak_squeezeblocks(t.get(), 1, state->ctx, SHA3_512_RATE);
...
void sha3_512(uint8_t* output, const uint8_t* input, size_t inlen)
{
    ...
    /* Absorb input */
    keccak_absorb(s.get(), SHA3_512_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t.get(), 1, s.get(), SHA3_512_RATE);
