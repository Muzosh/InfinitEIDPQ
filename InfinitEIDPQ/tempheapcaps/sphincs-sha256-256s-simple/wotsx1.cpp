 ...
 */
void wots_gen_leafx1(uint8_t* dest, const spx_ctx* ctx, uint32_t leaf_idx, void* v_info)
{
    struct leaf_info_x1* info = static_cast<struct leaf_info_x1*>(v_info);
    uint32_t* leaf_addr = info->leaf_addr;
    uint32_t* pk_addr = info->pk_addr;
    unsigned int i, k;
    uint8_t* pk_buffer = (uint8_t*)heap_caps_malloc((sizeof(uint8_t)*(SPX_WOTS_BYTES)), MALLOC_CAP_DEFAULT);
    uint8_t* buffer;
